...
const Despesas: React.FC = () => {
  ...
  async function loadData() {
    ...
    try {
      ...
      const response = await api.get(
        `despesas/listar.php?pagina=${page}&limite=15`,
      );

      const totalItemsApi = Number(response?.data?.totalItems ?? 0);
      if (totalItemsApi > 0) setTotalItems(totalItemsApi);

      const novos = Array.isArray(response?.data?.resultado)
        ? response.data.resultado
        : [];

      if (novos.length === 0) return;

      if (lista.length >= totalItemsApi && totalItemsApi > 0) return;

      const merged = [...lista, ...novos].sort((a: any, b: any) =>
        String(a.nome ?? "")
          .toLowerCase()
  ...
  async function Search() {
    if (!String(busca).trim()) {
      const response = await api.get(`despesas/listar.php?pagina=1&limite=15`);
      const itens = Array.isArray(response?.data?.resultado)
        ? response.data.resultado
        : [];
    ...
    const response = await api.get(`despesas/buscar.php?buscar=${busca}`);

    const itens = Array.isArray(response?.data?.itens)
      ? response.data.itens
      : [];
    const ordenado = [...itens].sort((a: any, b: any) =>
      String(a.nome ?? "")
          ...
          <table>
            ...
            <tbody>
              ${lista
                ...
                <tr>
                  <td>${item.nome || "N/A"}</td>
                  <td>${item.produtos || "N/A"}</td>
                  <td>${item.ativo || "N/A"}</td>
                </tr>
              `,
                )
                .join("")}
    ...
    <View style={styles.container}>
      ...
      <View style={styles.containerFloat}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate("NovaDespesa", { id_reg: "0" })}
        >
