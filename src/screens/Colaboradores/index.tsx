...
const Colaboradores: React.FC = () => {
  ...
  async function loadData() {
    try {
      const response = await api.get(
        `colaboradores/listar.php?pagina=${page}&limite=15`,
      );
  ...
  const handleEdit = (id: string) => {
    navigation.navigate("NovoColaborador", { id_reg: id });
  };

  const handleDelete = async (id: string) => {
    Alert.alert("Excluir", "Deseja realmente excluir este colaborador?", [
      { text: "Cancelar", style: "cancel" },
      {
        text: "Excluir",
        style: "destructive",
        onPress: async () => {
          await api.get(`colaboradores/excluir.php?id=${id}`);
  ...
  const handleParcelar = (id: string) => {
    navigation.navigate("ParcelarColaborador", { id_reg: id });
  };
  ...
  async function Search() {
    const response = await api.get(`colaboradores/buscar.php?buscar=${busca}`);
            ...
            <tbody>
              ${lista
                ...
                <tr>
                  ...
                  <td>${colaborador.telefone || "N/A"}</td>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.containerFloat}>
        <TouchableOpacity
          ...
          onPress={() =>
            navigation.navigate("NovoColaborador", { id_reg: "0" })
          }
        >
