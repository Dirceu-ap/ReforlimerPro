...
const Usuarios: React.FC = () => {
  ...
  async function loadData() {
    try {
      const response = await api.get(
        `usuarios/listar.php?pagina=${page}&limite=15`
      );
  ...
  async function Search() {
    const response = await api.get(`usuarios/buscar.php?buscar=${busca}`);
      ...
      <View style={{ paddingHorizontal: 15, flex: 1 }}>
        ...
        <View style={styles.containerFloat}>
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate("NovoUsuario", { id_reg: "0" })}
          >
