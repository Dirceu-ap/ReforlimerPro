...
const Categorias: React.FC = () => {
  ...
  async function loadData() {
    try {
      const response = await api.get(
        `categorias/listar.php?pagina=${page}&limite=15`,
      );
  ...
  async function Search() {
    const response = await api.get(`categorias/buscar.php?buscar=${busca}`);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.containerFloat}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate("NovaCategoria", { id_reg: "0" })}
        >
