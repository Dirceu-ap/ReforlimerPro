...
import CardCatDespesa from "../../components/CardCatDespesa";

const CatDespesa: React.FC = () => {
  ...
  async function loadData() {
    try {
      const response = await api.get(
        `catdespesa/listar.php?pagina=${page}&limite=15`,
      );
      ...
      merged.sort((a: any, b: any) =>
        String(a.nome ?? a.cat_despesa ?? "")
          .toLowerCase()
          .localeCompare(
            String(b.nome ?? b.cat_despesa ?? "").toLowerCase(),
            "pt-BR",
          ),
      );
      setLista(merged);
  ...
  const renderItem = function ({ item }: any) {
    return <CardCatDespesa data={item} />;
  };
  ...
  async function Search() {
    const response = await api.get(`catdespesa/buscar.php?buscar=${busca}`);
            ...
            <tbody>
              ${lista
                .map(
                  (item: any) => `
                <tr>
                  <td>${item.nome || item.cat_despesa || "N/A"}</td>
    ...
    <View style={styles.container}>
      ...
      <View style={{ paddingHorizontal: 15, flex: 1 }}>
        <View style={styles.containerSearch}>
          ...
          <TextInput
            style={styles.search}
            placeholder="Pesquisar Cat. de Despesa."
      ...
      <View style={styles.containerFloat}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate("NovaCatDespesa", { id_reg: "0" })}
        >
