...
const CardColaborador = ({ data }: DadosProps) => {
          ...
          onPress: async () => {
            try {
              const response = await api.get(
                `colaboradores/excluir.php?id=${id}`
              );
              showMessage({
                message: "Exclusão",
      ...
      data.email === undefined ? (
        ...
        <View>
          <View style={styles.cardDados}>
            ...
            <Text style={styles.cardInfo}>Função: {data.funcao || "-"}</Text>
            <Text style={styles.cardInfo}>
              Salário diário:{" "}
              {data.salario_diario ? `R$ ${data.salario_diario}` : "-"}
            </Text>
          ...
          <View style={styles.cardAcoes}>
            <TouchableOpacity
              onPress={() =>
                navigation.push("NovoColaborador", { id_reg: data.id })
              }
